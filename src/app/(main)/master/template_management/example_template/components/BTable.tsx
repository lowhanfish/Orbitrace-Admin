...
const BTable = () => {
    const [open, setOpen] = useState(false);
    return (
        <div className='flex flex-col bg-linear-to-r from-b-gray-1 to-50% to-b-gray-1/40 shadow-sm rounded-lg px-3 py-3 mt-2'>
            <table className='Btable w-full'>
                <thead>
                ...
                <tbody>
                    {Data.map((item, index) => (
                        <tr key={index}>
                            ...
                            <td>{item.address}</td>
                            <td>
                                <p className={\`w-full py-1 flex items-center justify-center rounded-2xl text-[12px] text-b-gray-6 \${
                                    item.status === "pending" ? "bg-b-yellow-5" : item.status === "approve" ? "bg-b-green-5" : "bg-b-red-5"
                                }\`}>
                                    {item.status}
                                </p>
